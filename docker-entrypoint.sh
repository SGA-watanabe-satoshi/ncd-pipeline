...
declare -a INPUT_FILES=(
    "gs://classification-data/*/*.gz",
    "gs://classification-data-2/*/*.gz",
    "gs://classification-data-4/*/*.txt",
    "gs://classification-data-data-on-demand/*/*.gz"
)
...
run_classifier_data_transfer_pipeline() {
    ...
    if [[ ( "$SKIP_INPUT_FILE_PRECHECK" == "true" ) || ( "$new_hash" != "$old_hash" ) ]]; then
        ...
        for segment_file in ${INPUT_FILES[@]}; do
